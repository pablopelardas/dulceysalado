<template>
  <Transition name="modal-fade">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8">
      ...
      <!-- Modal -->
      <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] md:max-h-[80vh] flex flex-col">
        <!-- Header - Fixed -->
        <div class="flex-shrink-0 px-4 md:px-6 py-4 border-b border-gray-100 rounded-t-2xl">
        ...
        <!-- Export Options - Scrollable -->
        <div class="flex-1 overflow-y-auto p-4 md:p-6">
          <div class="space-y-3">
            <!-- Copy to Clipboard -->
            <button 
              @click="copyToClipboard"
              class="w-full flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors cursor-pointer group"
            >
              <div class="w-10 h-10 md:w-12 md:h-12 bg-purple-500 rounded-full flex items-center justify-center text-white flex-shrink-0">
                <ClipboardDocumentIcon class="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <div class="flex-1 text-left min-w-0">
                <div class="font-medium text-gray-900 group-hover:text-purple-700 text-sm md:text-base">Copiar al portapapeles</div>
                <div class="text-xs md:text-sm text-gray-500">Copiar lista para pegar donde necesites</div>
              </div>
              <ChevronRightIcon class="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-purple-600 flex-shrink-0" />
            </button>
            ...
            <button 
              ...
              class="w-full flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-green-50 hover:bg-green-100 rounded-lg transition-colors cursor-pointer group border-2 border-green-200"
            >
              <div class="w-10 h-10 md:w-12 md:h-12 bg-green-600 rounded-full flex items-center justify-center text-white flex-shrink-0">
              ...
              <div class="flex-1 text-left min-w-0">
                <div class="font-medium text-gray-900 group-hover:text-green-700 flex items-center gap-2 text-sm md:text-base">
                  <span class="truncate">Enviar pedido a {{ EMPRESA_CONFIG.nombre }}</span>
                  <span class="px-2 py-1 text-xs bg-green-600 text-white rounded-full flex-shrink-0">Empresa</span>
                </div>
                <div class="text-xs md:text-sm text-gray-500">Enviar lista como pedido por WhatsApp</div>
              </div>
              <ChevronRightIcon class="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-green-600 flex-shrink-0" />
            </button>
            
            <!-- WhatsApp General -->
            <button 
              @click="exportToWhatsApp"
              class="w-full flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-green-50 hover:bg-green-100 rounded-lg transition-colors cursor-pointer group"
            >
              <div class="w-10 h-10 md:w-12 md:h-12 bg-green-500 rounded-full flex items-center justify-center text-white flex-shrink-0">
                <svg class="w-5 h-5 md:w-6 md:h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.106"/>
                </svg>
              </div>
              <div class="flex-1 text-left min-w-0">
                <div class="font-medium text-gray-900 group-hover:text-green-700 text-sm md:text-base">WhatsApp</div>
                <div class="text-xs md:text-sm text-gray-500">Enviar por WhatsApp a cualquier contacto</div>
              </div>
              <ChevronRightIcon class="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-green-600 flex-shrink-0" />
            </button>
            ...
            <button 
              @click="exportToEmail"
              class="w-full flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors cursor-pointer group"
            >
              <div class="w-10 h-10 md:w-12 md:h-12 bg-blue-500 rounded-full flex items-center justify-center text-white flex-shrink-0">
                <EnvelopeIcon class="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <div class="flex-1 text-left min-w-0">
                <div class="font-medium text-gray-900 group-hover:text-blue-700 text-sm md:text-base">Email</div>
                <div class="text-xs md:text-sm text-gray-500">Enviar por correo electrónico</div>
              </div>
              <ChevronRightIcon class="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-blue-600 flex-shrink-0" />
            </button>
            
            <!-- PDF File -->
            <button 
              @click="exportToPDF"
              class="w-full flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-red-50 hover:bg-red-100 rounded-lg transition-colors cursor-pointer group"
            >
              <div class="w-10 h-10 md:w-12 md:h-12 bg-red-500 rounded-full flex items-center justify-center text-white flex-shrink-0">
                <DocumentTextIcon class="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <div class="flex-1 text-left min-w-0">
                <div class="font-medium text-gray-900 group-hover:text-red-700 text-sm md:text-base">Documento PDF</div>
                <div class="text-xs md:text-sm text-gray-500">Descargar como archivo PDF</div>
              </div>
              <ChevronRightIcon class="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-red-600 flex-shrink-0" />
            </button>
          </div>
        </div>
        
        <!-- Footer - Fixed -->
        <div class="flex-shrink-0 px-4 md:px-6 py-3 bg-gray-50 rounded-b-2xl border-t border-gray-100">
