...
useHead({
  ...
  meta: [
    { name: 'description', content: 'Configura qué agrupaciones aparecen como novedades en el catálogo público' }
  ]
})
...
const onSave = async (newVisibleIds: number[]) => {
  ...
  try {
    await updateNovedades(empresaSeleccionada.value.id, newVisibleIds)
    
    // Mostrar toast de éxito
    const toast = useToast()
    toast.add({
      title: 'Novedades actualizadas',
      description: `Se han actualizado las novedades para ${empresaSeleccionada.value.nombre}`,
      color: 'green'
    })
  } catch (err) {
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8">
        <nav class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
          ...
          <span class="text-gray-900 dark:text-gray-100">Gestionar Novedades</span>
        </nav>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-2 text-gray-600 dark:text-gray-400">
              Configura qué agrupaciones aparecen como novedades en el catálogo público
            </p>
          </div>
