...
import OfertasDragDrop from '~/components/marketing/OfertasDragDrop.vue'
...
useHead({
  title: 'Configurar Ofertas - Empresa',
  meta: [
    { name: 'description', content: 'Configura las ofertas visibles para la empresa' }
  ]
})
...
const onSave = async (newVisibleIds: number[]) => {
  try {
    await updateOfertas(empresaId, newVisibleIds)
    await fetchOfertas(empresaId)
    
    // Mostrar toast de éxito
    const toast = useToast()
    toast.add({
      title: 'Ofertas actualizadas',
      description: `Se han actualizado las ofertas para ${empresa.value?.nombre}`,
      color: 'green'
    })
  } catch (err) {
...
const goBack = () => {
  router.push('/admin/marketing/ofertas/empresas')
}

// Cargar datos iniciales
onMounted(async () => {
  if (isNaN(empresaId)) {
    await router.push('/admin/marketing/ofertas/empresas')
    return
  }
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8">
        <nav class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
          ...
          <NuxtLink to="/admin/marketing/ofertas/empresas" class="hover:text-gray-900 dark:hover:text-gray-100">
            Ofertas
          </NuxtLink>
          <UIcon name="i-heroicons-chevron-right" class="h-4 w-4" />
          ...
          <div>
            ...
            <p class="mt-2 text-gray-600 dark:text-gray-400">
              <template v-if="empresa">
                Configura las ofertas visibles para: <strong>{{ empresa.nombre }}</strong>
              </template>
              <template v-else>
                Configura las ofertas visibles para la empresa
              </template>
            </p>
          </div>
