...
useHead({
  title: 'Configurar Agrupaciones por Empresa',
  meta: [
    { name: 'description', content: 'Selecciona una empresa para configurar sus agrupaciones visibles' }
  ]
})
...
const fetchEmpresas = async () => {
  ...
  try {
    const response = await api.get('/api/Companies', {
      params: {
        page: 1,
        pageSize: 100, // Traer todas las empresas
        includeInactive: true // Incluir inactivas para mostrar todas
      }
    })
    ...
    // Incluir todas las empresas (principal y clientes) para configuración de agrupaciones
...
const configureEmpresa = (empresaId: number) => {
  router.push(`/admin/agrupaciones/${empresaId}/configurar`)
}
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8">
        <nav class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
          ...
          <span class="text-gray-900 dark:text-gray-100">Configurar por Empresa</span>
        </nav>
        
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">
              Configurar Agrupaciones por Empresa
            </h1>
            <p class="mt-2 text-gray-600 dark:text-gray-400">
              Selecciona una empresa para configurar qué agrupaciones serán visibles en su catálogo
            </p>
          </div>
